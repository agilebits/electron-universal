...
const makeUniversalApp = async (opts) => {
    ...
    try {
        ...
        for (const plistFile of plistFiles) {
            ...
            if (injectAsarIntegrity &&
                x64ParsedPlist &&
                x64ParsedPlist.ElectronAsarIntegrity &&
                arm64ParsedPlist &&
                arm64ParsedPlist.ElectronAsarIntegrity) {
                const { ElectronAsarIntegrity: x64Integrity } = x64ParsedPlist, x64Plist = __rest(x64ParsedPlist, ["ElectronAsarIntegrity"]);
                const { ElectronAsarIntegrity: arm64Integrity } = arm64ParsedPlist, arm64Plist = __rest(arm64ParsedPlist, ["ElectronAsarIntegrity"]);
